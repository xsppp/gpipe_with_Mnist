...
class GPipeLenet5PipeLine(gpipe.PipeliningLayer):
    ...
    def Params(cls):
        p = super().Params()
        p.Define('filter_shapes',[(0, 0, 0, 0)],'the filter shapes for the conv layers')
        p.Define('window_shapes',[(0, 0)],'window shapes for the pool layers')
        ...
        p.Define('sm1',layers_with_gpipe.GPipeImageProcessingSoftmaxLayer.Params(),'')
    ...
    def __init__(self, params):
      ...
      batch_norm=p.batch_norm)
      p.pool1.Set(name='pool1',window_shape=p.window_shapes[0],window_stride=p.window_shapes[0])
      p.pool2.Set(name='pool2',window_shape=p.window_shapes[1],window_stride=p.window_shapes[0])
      shape = [256] + list(p.input_shape)
...
class GPipeIm(base_layer.BaseLayer):
    @classmethod
    def Params(cls):
      p = super().Params()
