...
class GPipeLeNet5(Base):
    ...
    BATCH_SIZE = 32
    GPUS = 3
    SPLITS = [2 * (i + 1) for i in range(GPUS)]
    LAYERS = SPLITS[-1]
    NUM_MICRO_BATCHES = 16

    def Task(self):
        p = classifier.GPipeModel.Params() #TO BE CHANGED
        p.name = 'gpipelenet5'
        p.im = im_layers.GPipeIm.CommonParams(
        filter_shapes = [(5, 5, 1, 20),(5,5,20,50)],
        window_shapes = [(2, 2), (2, 2)],
        batch_norm = self.BN,
