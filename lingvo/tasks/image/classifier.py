...
from lingvo.tasks.image import layers as im_layers
import numpy as np
...
class GPipeModel(BaseClassifier):
    """Similar to ModelV1 except with GPipe"""
    """call to params() will call the base classifier params"""
    @classmethod
    def Params(cls):
      p = super().Params()
      p.Define('im',im_layers.GPipeIm.CommonParams(),'the layers of the image model you want')
      tp = p.train
      tp.learning_rate = 1e-4  # Adam base LR.
      tp.lr_schedule = (
          schedule.LinearRampupExponentialDecayScaledByNumSplitSchedule.Params()
    ...
    def __init__(self, params):
      ...
      super().__init__(params)
      p = self.params
      p.im.pipelinestack.input_shape = p.input.data_shape
      self.CreateChild('pipelinedmodel',p.im)



    def FPropTower(self, theta, input_batch):
      print('starting f prop tower')
      p = self.params
      batch = tf.shape(input_batch.data)[0]
      height, width, depth = p.input.data_shape
      act = tf.reshape(input_batch.data, [batch, height, width, depth])
