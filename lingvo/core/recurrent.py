...
class _Recurrent:
  ...
  def __init__(self,
    ...
    def BackwardLoopBody(loop_state):
      ...
      '''temp1 = sa.attach("timing")
      temp2 = sa.attach("info")
      temp3 = sa.attach("stack_index")
      temp1[temp3[0]] = time.time()
      temp2[temp3[0]] = "grad comp. started"
      temp3[0] +=1'''
      t = loop_state.t
  ...
  # to Bak() which adds to the captured list.
      ...
      #print('input layer gradient comp. finished at:',time.time())
      return loop_state
...
class _Output:
  ...
  def Compute(self):
    ...
    def OutputFn(theta, state0, inputs):
      del inputs
      #print('output layer receiving at:',time.time())
      #temp1[temp3[0]] = time.time()
      #temp2[temp3[0]] = "starting output layer receiving"
      #temp3[0] +=1
      inputs = self._in_links.Transform(lambda l: l.fwd.Recv())
      #print('output finished receiving at:',time.time())
      state1, extras = self._cell_fn(theta, state0, inputs)
      py_utils.AssertIsCompatible(state1, state0)
      py_utils.AssertIsCompatible(extras, self._extras)
...
def _StackedRecurrent(devices, cell_fns, cell_grads, cell_outs, cell_out_grads,
  ...
  #print('finished evaluating expected outputs of layers at:',time.time())
  ...
  #print('starting symbolic creation of link between device 0 and device 1 at time:',time.time())
  # Builds the input layer.
  out_links = _CreateLinks(expected_output_by_layers[0].xs,
                           DevicePair(devices[0], devices[1]))
  #print('finished creation of symbolic link btwn device 0 and 1 at:',time.time())
  # Enable accumulators. Note that this must happen prior to the initial
  # _AugmentState() below or it will initialize with defaults.
  for accumulator_layer in accumulator_layers:
    if accumulator_layer:
      accumulator_layer.accumulators.Transform(lambda x: x.Enable())
  #print('building of input layers at:',time.time())
  inp_l = _Input(
      cell_fn=cell_fns[0],
      cell_grad=cell_grads[0],
  ...
  layers += [inp_l]
  #print('building done for input layers at:',time.time())
  ...
  for i in range(1, num_layers - 1):
    ...
    mid_l = _Middle(
        ...
        unused_acc_state=unused_acc_state)
    layers += [mid_l]
  #print('linking of middle layers done at:',time.time())
  ...
  #print('actual forward prop with data transfer starting at:',time.time())
  anchor = 0
  final_states = []
  ...
  #print('actual fwd prop ended at:',time.time())
  ...
  #print('releasing of final layer output starting at:',time.time())
  ...
  py_utils.ResetStepSeed(
      py_utils.GenerateSeedFromName(tf.no_op(name='new_step_seed').name))
  #print('releasing of final layer outputs ended at:',time.time())
